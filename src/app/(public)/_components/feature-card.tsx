...
export default function FeatureCard({ title, description, image, features, link }: Props) {
  return (
    <Link href={link.href}>
      <Card className="group h-full w-screen max-w-lg overflow-hidden select-none max-lg:max-w-md max-sm:max-w-sm lg:pt-0">
        <div className="bg-muted relative flex h-84 w-full items-center justify-center overflow-hidden rounded-2xl border-8 border-white px-12 transition-all duration-300 group-hover:h-72 max-lg:hidden">
          <Image
            ...
            alt={image.alt}
            className={
              "pointer-events-none relative z-50 scale-125 rounded-xl border object-contain opacity-60 shadow-lg contrast-150 transition-all duration-300 !select-none group-hover:opacity-100 hover:mt-2"
            }
            style={{
                  ...
                  {feature.shopifyRelated && (
                    <Tooltip>
                      <TooltipTrigger className="ml-2 inline translate-y-0.5 opacity-50 transition-all duration-300 group-hover:opacity-100">
                        <Image src="/images/shopify-logo.png" width={16} height={16} alt="Shopify Logo" />
                      </TooltipTrigger>
                  ...
                  {feature.wordpressRelated && (
                    <Tooltip>
                      <TooltipTrigger className="ml-2 inline translate-y-1.5 opacity-50 transition-all duration-300 group-hover:opacity-100">
                        <Image src="/images/wordpress-logo.webp" width={24} height={24} alt="WordPress Logo" />
                      </TooltipTrigger>
                      <TooltipContent side="right" className="pointer-events-none">
                        {feature.wordpressRelated}
                      </TooltipContent>
